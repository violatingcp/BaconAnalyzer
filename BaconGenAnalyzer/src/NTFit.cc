...
NTFit::NTFit(int iType, std::string iLikelihoodFile, NTFit::Options iOption,NTFit::CalcOptions iCalcOption,int iNEvents,bool iCorrectMet) { 
  f13Body = false;
  f23Body = 0;
  if(iType == -1)  f23Body = -1;
  if(iType ==  1)  f13Body = true;
  if(iType ==  2) {f13Body = true; f23Body = 1;}
  if(iType ==  3) f23Body = 1;
  fNEvents    = iNEvents;
...
TLorentzVector NTFit::NeutrinoToy(TRandom2 &iRand,double &iX,double &iCosPhi,TLorentzVector &iTau,double iPt,double iPhi,double iEta,double iMLep,bool i3Body,double iXMin) { 
  ...
  if(fOption == kFlat2) { 
    ...
    if(!i3Body )  lY=acos(iRand.Uniform(-1.,1.));
  ...
  if(fOption == kME) { 
    if( i3Body ) {lY=acos(iRand.Uniform(-1.,1.)); lX=iRand.Uniform(0.2,0.99);} //{f2D->GetRandom2(lX,lY); lX*=2.;}// lY = acos(lY);}
    double lCosY = lVec.P()/fMTau/2.;
    if(lCosY > 1) lCosY = 1;
    lCosY = sqrt(1.-lCosY*lCosY);
    //double lRandCos = iRand.Uniform(lCosY,1) * (1-2*(iRand.Uniform(-1,1) < 0));
    double lRandCos = iRand.Uniform(-1.,1.);
    if(!i3Body )  lY=acos(lRandCos);
    if( i3Body )  fTempWeight *= fF1->Eval(lX,lY); // lY = acos(lY);}
    //if(!i3Body )  fTempWeight *= fF2->Eval(lY);
  }
  ...
  //std::cout << " ---> " << lPRest << " -- " << lGamma << " -- " << lVec.Pt() << " -- " << iMLep << std::endl; 
...
void NTFit::calculate() { 
  ...
  fTau1 .SetPtEtaPhiM(fPt1, fPhi1,fEta1,fM1); 
  fTau2 .SetPtEtaPhiM(fPt2, fPhi2,fEta2,fM2); 
...
void NTFit::buildFourVector(int iNEvents,double iPt1,double iEta1,double iPhi1,double iM1,double iPt2,double iEta2,double iPhi2, double iM2, double iMEX,double iMEY,bool iClear) { 
  ...
  for(int i0 = 0; i0 < iNEvents; i0++) {
    ...
    if(f23Body  > -1) lNu2 = NeutrinoToy(*fRand,fX2,fCosPhi2,lTau2,iPt2,iPhi2,iEta2,iM2,f23Body,0.);
    if(f23Body == -1) lNu2.SetPtEtaPhiM(0.1,0,0,0);
    ...
    double lWeight      = fGaus->Eval(sqrt(lDiff))*lN; 
    double lCosMin      = TMath::Min(fCosPhi1,fCosPhi2);
    double lCosMax      = TMath::Max(fCosPhi1,fCosPhi2);
    fTempWeight *= (1+lCosMax)*(1+lCosMax);//*(1-lCosMax);//+lCosMax*lCosMax*0.5;
    fTempWeight *= (1-lCosMin)*(1-lCosMin);//lCosMin*lCosMin;//(1+lCosMin)*(1-lCosMin);//)+lCosMin*lCosMin*0.5;
    if(fOption == kFlat2 || fOption == kME) {lWeight *= fTempWeight; fTempWeight = 1.;}
    if(isnan(lWeight) || isinf(lWeight)) continue;
    if(fCalcOption == kMean) fillPars(lWeight,lSys,lTau1,lTau2);
    if(fCalcOption == kMax)  maxPars (lWeight,lSys,lTau2,lTau2);
    histPars(lWeight,lSys); //To get the unc
    //if(lWeight > 10) std::cout << "===> " << lWeight << " -- " << lSys.M() << std::endl;
  } //End of loop
  return;
}
...
TLorentzVector NTFit::tau1Vec() { 
  return fTau1;
}
TLorentzVector NTFit::tau2Vec() { 
  return fTau2;
}
///Tools
...
double NTFit::gamma(TLorentzVector iVec,double iP,double iTheta,double iMLep) { 
  ...
  double lEBoost = sqrt(iVec.P()*iVec.P()-iP*iP*sin(iTheta)*sin(iTheta));
  double lGam    = -lEBoost * iP*cos(iTheta) + lE * sqrt(-lE*lE+iP*cos(iTheta)*iP*cos(iTheta)+lEBoost*lEBoost);
  lGam/=(lE*lE-iP*iP*cos(iTheta)*cos(iTheta));
  
  //double lGam1    = -lEBoost * iP*cos(iTheta) - lE * sqrt(-lE*lE+iP*cos(iTheta)*iP*cos(iTheta)+lEBoost*lEBoost);
  ////lGam1/=(lE*lE-iP*iP*cos(iTheta)*cos(iTheta));
  return lGam;
}
...
///////////Stupid Bookkeeping
void NTFit::fillPars(double iWeight, TLorentzVector &iSys,TLorentzVector &iTau1,TLorentzVector &iTau2) { 
  fTotWeight   += iWeight;
  ...
  fPt1         += iWeight*iTau1.Pt();
  fEta1        += iWeight*iTau1.Eta();
  fPhi1        += iWeight*iTau1.Phi();
  fM1          += iWeight*iTau1.M();

  fPt2         += iWeight*iTau2.Pt();
  fEta2        += iWeight*iTau2.Eta();
  fPhi2        += iWeight*iTau2.Phi();
  fM2          += iWeight*iTau2.M();
}
void NTFit::divide() { 
  //Usefull Polarization variables
...
void NTFit::maxPars(double iWeight, TLorentzVector &iSys,TLorentzVector &iTau1,TLorentzVector &iTau2) { 
