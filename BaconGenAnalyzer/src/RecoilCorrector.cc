...
void RecoilCorrector::metDistribution(double &iMet,double &iMPhi,double iGenPt,double iGenPhi,
  ...
  double pU1       = -iGenPt;//CorrVal(iGenPt,iU1RZDatFit->Eval(iGenPt),PFU1); //iU1RZDatFit->Eval(iGenPt);//CorrVal(iGenPt,iU1RZDatFit->Eval(iGenPt),PFU1);
  ...
  double lU1U2   = 0.0;//
  if(iU1U2Corr != 0) lU1U2 = iU1U2Corr->Eval(iGenPt)*0.5;
  lU1U2 = 0.;
  ...
  //cout << "===> " << pU1 << " -- " <<iGenPt << " -- " << pU2 << " -- Rest : " << pSigma1_1 << " -- " <<pSigma2_1 << " -- " << pVal1_1 << " -- " << pVal2_1 << endl;
  ...
  iU1   = pU1; 
  iU2   = pU2;
  double lCovU1 = pSigma1_1*pSigma1_1;
  double lCovU2 = pSigma2_1*pSigma2_1;
  
  TLorentzVector lVec1; lVec1.SetPtEtaPhiM(pU1,0,iGenPhi,0); 
  TLorentzVector lVec2; lVec1.SetPtEtaPhiM(pU2,0,iGenPhi+TMath::Pi()/2.,0); 
  lVec1 += lVec2;
  double lUPhi = lVec1.Phi();
  double lCos2 = cos(lUPhi)*cos(lUPhi);
  double lSin2 = sin(lUPhi)*sin(lUPhi);
  (*fCov)(0,0)   =  lCovU1*lCos2+lCovU2*lSin2;
  (*fCov)(1,0)   = -lCovU1*sin(lUPhi)*cos(lUPhi)+lCovU2*sin(lUPhi)*cos(lUPhi);
  (*fCov)(0,1)   =  (*fCov)(1,0);
